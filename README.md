# <h1 align = "center"> Projeto de Portfólio Mobile-Friendly </h1>

<div align = "center">
<img src = ".\assets\site-celular.png" alt = "Imagem do site" height = "600">
</div>

### Status do Projeto: Concluído ✔

## 🗂 Sumário

- [📝 Descrição do Projeto](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao-Mobile--Projeto-N6-2025#-descri%C3%A7%C3%A3o-do-projeto)
- [💻 Acesso ao Projeto](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao-Mobile--Projeto-N6-2025#-acesso-ao-site)
- [🎯 Objetivos do Curso](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao-Mobile--Projeto-N6-2025#-objetivos-do-curso)
- [⏲ Versão Anterior do Projeto](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao-Mobile--Projeto-N6-2025#-vers%C3%A3o-anterior-do-projeto)
- [🔗 Protótipo do Projeto no Figma](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao-Mobile--Projeto-N6-2025#-prot%C3%B3tipo-do-projeto-no-figma)
- [💻 Tecnologias Utilizadas](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao-Mobile--Projeto-N6-2025#-tecnologias-utilizadas)
- [⚙ Funcionalidades e Inovações](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao-Mobile--Projeto-N6-2025#-funcionalidades-e-inova%C3%A7%C3%B5es)
- [📂 Estrutura do Projeto](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao-Mobile--Projeto-N6-2025#-estrutura-do-projeto)
- [📷 Imagens do Projeto](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao-Mobile--Projeto-N6-2025#-imagens-do-projeto)
- [✨Créditos](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao-Mobile--Projeto-N6-2025#-cr%C3%A9ditos)

## 📝 Descrição do Projeto

O atual projeto define a aplicação web de um portfólio feito em **HTML** e **CSS** com base em um protótipo do Figma fornecido pelos professores do curso ***HTML e CSS:Trabalhando com responsividade e publicação de projetos*** (link nos créditos finais). <br>

O objetivo é o de aplicação prática dos conteúdos de responsividade no CSS com a abordagem mobile-friendly. Para isso, foi utilizado o Media Querry para definir quais estilizações de espaçamento seriam aplicadas se o dispositivo do usuário tivesse um width menor que o padrão, o qual poderia quebrar a aplicação. Outra inovação desse site é o uso de medidas relativas de espaçamento, não mais as absolutas. Isso permite que o layout fique responsivo a diferentes telas de dispostivos, como tablets e celulares. <br>

Portanto, através da possibilidade de se acessar o site também em dispositívos móveis, o projeto se torna acessível para mais pessoas, o que segue as práticas do mercado real de escalabilidade e produção web. <br>

## 💻 Acesso ao Site 

Você pode acessar o projeto acessando [esse link do Vercel](https://projeto-de-portfolio.vercel.app) <br>

## 🎯 Objetivos do Curso

- Saiba como tornar seu site responsivo para diferentes telas;
- Descubra o que é como usar a Media Query em seus projetos;
- Crie uma conta no GitHub para armazenar seu código na nuvem;
- Entenda a diferença de medidas absolutas e relativas;
- Publique seu projeto finalizado na web e mostre para todo o mundo. <br>

## ⏲ Versão Anterior do Projeto

A versão anterior desse site pode ser encontrada [aqui](https://github.com/Victor-M-S-Rodrigues07/Projeto-Site-de-Apresentacao--Projeto-N5-2025). <br>

> [!NOTE]
> Essa é uma série de projetos, no qual em cada versão aplica-se novos conceitos e aprendizados. Foi proposto separar em repositórios diferentes para fins de melhor registro das evoluções e para paridade com cada curso da formação, dado que criar commits sequenciais, apesar de recomendado pelas práticas de mercado, não satisfaria esses objetivos do estudo sequencial do curso.

## 🔗 Protótipo do Projeto no Figma 

Você pode acessar o protótipo atualizado do projeto no Figma clicando [aqui](https://www.figma.com/community/file/1410377948870595725). <br>

> [!IMPORTANT]
> É necessário ter uma conta no Figma para ter acesso com mais detalhes aos elementos do protótipo. Para criá-la, basta clicar em Sign-Up e, sem seguida, inserir as informações do site. Lembrando que é uma ferramenta gratuita.

## 💻 Tecnologias Utilizadas

![HTML](https://img.shields.io/badge/HTML-0?style=for-the-badge&logo=html5&logoColor=%23FFFFFF&color=%23E34F26)
![CSS](https://img.shields.io/badge/CSS-0?style=for-the-badge&logo=css&logoColor=%23FFFFFF&color=%230022FF)
![Figma](https://img.shields.io/badge/Figma-0?style=for-the-badge&logo=figma&logoColor=white&color=%2305C7F2)
![Git](https://img.shields.io/badge/Git-0?style=for-the-badge&logo=git&logoColor=%23FFFFFF&color=%23F05032)
![GitHub](https://img.shields.io/badge/GitHub-0?style=for-the-badge&logo=github&logoColor=%23FFFFFF&color=%23181717) <br>

##  ⚙ Funcionalidades e Inovações

- Uso de medidas relativas (em, rem, %) no lugar de unidades de medidas absolutas (px);
- Media Querry para criar responsividade;
- Deploy da aplicação no Vercel. <br>

## 📂 Estrutura do Projeto

```
/assets
/styles
    style.css

about.html
curriculo.html
index.html
README.md
```

## 📷 Imagens do Projeto

<div align = "center">

<img src = ".\assets\site-desktop-home.png" alt = "Imagem do site na aba 'home' em formato de PC" height = "500"> 
<br>

<img src = ".\assets\site-desktop-sobre-mim.png" alt = "Imagem do site na aba 'Sobre Mim' em formato de PC" height = "500">
<br>

<img src = ".\assets\site-celular.png" alt = "Imagem do site na aba 'home' em formato mobile" height = "500">
<br>

</div>

## ✨ Créditos

<img src = "https://avatars.githubusercontent.com/u/187053289?v=4" width = 100px> <br>
<strong> Victor </strong>

<a href = "https://www.linkedin.com/in/victor-m-rodrigues/"><img alt="Static Badge" src="https://img.shields.io/badge/LinkedIn-0?style=for-the-badge&logoColor=%23FFFFFF&color=%230077B5&link=https%3A%2F%2Fwww.linkedin.com%2Fin%2Fvictor-m-rodrigues%2F"></a> 
<a href = "https://github.com/Victor-M-S-Rodrigues07/"><img alt="Static Badge" src="https://img.shields.io/badge/GitHub-0?style=for-the-badge&logo=github&logoColor=%23ffffff&labelColor=%23000000&color=%2300000000"></a>
<a href = "https://cursos.alura.com.br/user/victorvicmr"><img alt="Static Badge" src="https://img.shields.io/badge/Alura-0?style=for-the-badge&color=%23100D36"></a>
<a href = "https://cursos.alura.com.br/course/html-css-responsividade-publicacao-projetos"> <img alt="Static Badge" src="https://img.shields.io/badge/HTML%20e%20CSS%3A%20Trabalhando%20com%20responsividade%20e%20publica%C3%A7%C3%A3o%20de%20projetos-0?style=for-the-badge&label=Curso&color=%23D25075"> </a>

🇧🇷 - 2025
